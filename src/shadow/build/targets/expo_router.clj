(ns shadow.build.targets.expo-router
...
(defn configure
  [{:keys [worker-info] :as state} mode {:keys [chunks root-ns output-dir] :as config}]
  (let [output-dir
        (io/file output-dir)
...
(defn fake-expo-fs [state ns-root]
  (let [re (re-pattern (str "^" ns-root))
        defs (->> state :compiler-env :cljs.analyzer/namespaces vals
                  (into #{}
                        (comp
                          (mapcat (comp vals :defs))
                          (filter (comp :export :meta))
                          (filter (comp #(str/starts-with? (str %) ns-root) :name))
                          (map (juxt :name :file)))))]
    (->> defs
         (map (fn [[sym path]]
                [(-> path
...
(defn inject-expo-context [state mod]
  (when-let [ns-root (some-> state :shadow.build/config :root-ns str (str/split #"\.") first)]
    (let [ns-root-munged (-> ns-root cljs-comp/munge str)
          js-str (bootstrap-expo-context (->js-object (fake-expo-fs state ns-root-munged)))]
...
(defn flush [state mode config]
  (case mode
    ...
    :release
    (-> state
        (update :shadow.build.closure/modules (partial mapv #(inject-expo-context state %)))
